...
public class FightManager {
    ...
    public static void goFightChallengeCharaStatue(Chara chara, int menpai) {
        ...
        T_FightObject t_fightObject = GameData.that.baseFightObjectService.findOneByName(GameUtil.getZhangMenName(menpai));
    ...
    public static void goFightMapGuardian(String npcName, Chara chara, List<CharaStatue> defCharaStatueList) {
        ...
        for(i = 0; i < session.gameTeam.duiwu.size(); ++i) {
            ...
            CharaStatue charaStatue = new CharaStatue();
            charaStatue.copyFrom(session.gameTeam.duiwu.get(i));
            charaStatue.copyJiNengList(fightObject.skillsList);
        ...
        for(CharaStatue defCharaStatue:defCharaStatueList){
            ...
            if(null!=defCharaStatue.petbeibao){
                fightObject = new FightObject(defCharaStatue.petbeibao);
                fightObject.pos = (Integer)MONSTER_POS.get(num)+5;
                fightObject.fid = fc.id++;
                fightObject.type = 4;
                fightObject.skillsList = defCharaStatue.petJiNengList;
                monsterTeam.add(fightObject);
        ...
        fc.success = (fightContainer)->{
            MapGuardianService.onChallengeSuccess(npcName, attCharaStatueList, defCharaStatueList);
        };
        fc.teamList.add(ft);
        fc.teamList.add(monsterTeam);
        listFight.add(fc);
...
//                        vo_19959_0.vid = 0;
//                        vo_19959_0.para = 0;
//                        GameObjectChar.send(new MSG_C_ACTION(), vo_19959_0, gameObjectChar.chara.id);

//                        Vo_7655_0 vo_7655_0 = new Vo_7655_0();
//                        vo_7655_0.id = 0;
//                        GameObjectChar.send(new MSG_C_END_ACTION(), vo_7655_0, gameObjectChar.chara.id);
        if(victimObject.isPet()){
            GameObjectChar gameObjectChar = GameObjectCharMng.getGameObjectChar(victimObject.cid);
            if(null!=gameObjectChar && gameObjectChar.isOnline() && gameObjectChar.chara.petLueZhenId>0 && gameObjectChar.chara.petLueZhenId!=victimObject.id){
                Petbeibao petbeibao = gameObjectChar.chara.getLueZhenPet();
                if(null!=petbeibao){
                        FightObject fightObject = new FightObject(petbeibao);
    ...
    private static void sendOver(FightContainer fightContainer) {
        ...
        while(var2.hasNext()) {
            ...
            if (fightObject.type == 1) {
                ...
                GameUtil.notifyFightPet(session);
            } else if (fightObject.type == 2) {
                fightObject.max_shengming = fightObject.shengming;
                fightObject.update(fightContainer);
            }
        ...
        while(true) {
            ...
            if (!monsterNotAllDead) {
                ...
                if(fightContainer.isBattleType(BattleType.CHALLENGE_LEADER)){
                    ...
                    //雕像
                    CharaStatueService.saveCharaStature(zhangMenName, fightContainer.charaStatue);
                    //刷新npc视野信息
                    Npc npc = GameData.that.baseNpcService.findOneByName(zhangMenName);
                ...
                for(i = 0; i < GameLine.gameShuaGuai.shuaXing.size(); ++i) {
                    if (guaiwu != null && ((Vo_65529_0)GameLine.gameShuaGuai.shuaXing.get(i)).name.equals(((FightObject)guaiwu.get(0)).str)) {
                        ...
                        GameObjectChar.sendduiwu(new MSG_DISAPPEAR_Chara(), ((Vo_65529_0)GameLine.gameShuaGuai.shuaXing.get(i)).id, chara1.id);
