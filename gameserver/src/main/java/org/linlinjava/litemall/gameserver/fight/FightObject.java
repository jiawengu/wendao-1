...
//        }
    ...
    public FightObject(T_FightObject t_fightObject, Integer id){
        int metal = GameUtil.getMetal(t_fightObject.getPolar());
        this.bossid = id;
        this.str = t_fightObject.getName();
        this.guaiwulevel = 1;
    ...
    public boolean canAtta() {
        ...
        if (this.isSuoLing() || this.hasBuffState(3856)) {
            canbe = false;
        }
...
//        FightManager.send(fightContainer, new MSG_AUTO_TALK_DATA(), vo_32913_0);
    ...
    /**
     * 是否是锁灵状态
     * 该状态下所有战斗指令无效，但可以使用道具；死亡后锁灵状态马上解除但当前回合无法被复活
     * @return
     */
    public boolean isSuoLing(){
        return this.hasBuffState(3844);
    }
