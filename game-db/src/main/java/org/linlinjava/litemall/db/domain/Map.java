...
public class Map implements Cloneable, Serializable {
    ...
    public void andLogicalDeleted(boolean deleted) {
        this.setDeleted(deleted ? Map.Deleted.IS_DELETED.value() : Map.Deleted.NOT_DELETED.value());
    }
    ...
    static {
        IS_DELETED = Map.Deleted.IS_DELETED.value();
        NOT_DELETED = Map.Deleted.NOT_DELETED.value();
    }
    ...
    public static enum Column {
        ...
        public static Map.Column[] excludes(Map.Column... excludes) {
            ArrayList<Map.Column> columns = new ArrayList(Arrays.asList(values()));
            if (excludes != null && excludes.length > 0) {
                columns.removeAll(new ArrayList(Arrays.asList(excludes)));
            }

            return (Map.Column[])columns.toArray(new Map.Column[0]);
        }
